...
async function seed() {
  ...
  for (let i = 0; i < sampleListings.length; i++) {
    ...
    const citySlug = listingData.address.includes('Prishtinë') || listingData.address.includes('Prishtinës')
      ? 'prishtine'
      : listingData.address.includes('Prizren') ? 'prizren'
      : listingData.address.includes('Gjilan') ? 'gjilan'
      : listingData.address.includes('Pejë') ? 'peje'
      : 'prishtine';
