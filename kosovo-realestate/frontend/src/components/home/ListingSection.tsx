...
export default function ListingSection({ title, subtitle, eyebrow, listings, isLoading, viewAllHref, skeletonCount = 8, hideHeader }: ListingSectionProps) {
  const { t } = useTranslation('listingSection');
  return (
    <section className={hideHeader ? '' : 'section'}>
      <div className="container-page">
        ...
        {hideHeader && (
          <div className="flex justify-end mt-4">
            <Link href={viewAllHref} className="link flex items-center gap-1 text-sm">
              {t('viewAll')} <ArrowRight className="w-4 h-4" />
            </Link>
          </div>
        )}
